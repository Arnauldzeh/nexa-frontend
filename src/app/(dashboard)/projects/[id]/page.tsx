...
import {
  ...
  removeTrackedDocumentByFile,
  restoreTrackedDocumentFromTrash,
  rollbackApprovalWithNewVersion,
...
function mergeTrackedIntoDocs(
  phaseDocs: DocData[],
  projectId: string,
  phase: string,
): DocData[] {
  return phaseDocs.map((doc) => {
    const tracked = getTrackedDocumentsLatest(projectId, phase, doc.name);
...
function buildProjectFromStore(id: string): typeof DEFAULT_PROJECT {
...
export default function ProjectConfigPage() {
  ...
  const projectId =
    typeof params.id === "string"
      ...
      : Array.isArray(params.id)
        ? params.id[0]
        : "";
  const PROJECT = useMemo(() => buildProjectFromStore(projectId), [projectId]);
  ...
  useEffect(() => {
    ...
    setEtudeDocs(mergeTrackedIntoDocs(init.etude, PROJECT.id, "etude"));
    setPassationDocs(
  ...
  const LivrablesTable = ({
    ...
    const gridStyle = {
      gridTemplateColumns:
        "minmax(250px,2.5fr) 90px 100px minmax(120px,1.5fr) 130px",
    };
    return (
          ...
          return (
            <div key={idx}>
              ...
              >
                ...
                {/* Action (folder = Déposer uniquement) */}
                <div
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <div className="text-[11px] text-[var(--text-secondary)] font-medium mt-0.5">
                GED Projet • {PROJECT.description}
              </div>
            </div>
          </div>
          <div className="flex gap-7">
            ...
            <div className="text-right">
              ...
              <div className="w-16 h-1 bg-[var(--bg-inset)] rounded-full mt-1 overflow-hidden">
                <div
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          ...
          {/* Component selected — Phase tabs + Documents */}
            ...
            <>
              ...
              <div className="flex items-center gap-1 mb-6 bg-[var(--bg-surface)] rounded-[var(--radius-md)] border border-[var(--border-default)] p-1 w-fit shadow-[var(--shadow-sm)]">
                <button
                  ...
                  className={`px-5 py-2 rounded-[var(--radius-sm)] text-xs font-bold transition-all ${currentPhase === "etude" ? "bg-[var(--text-primary)] text-[var(--text-inverted)] shadow-[var(--shadow-sm)]" : "text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]"}`}
                >
                  Étude
                </button>
                <button
                  ...
                  className={`px-5 py-2 rounded-[var(--radius-sm)] text-xs font-bold transition-all ${currentPhase === "passation" ? "bg-[var(--text-primary)] text-[var(--text-inverted)] shadow-[var(--shadow-sm)]" : "text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]"}`}
                >
                  Passation
                </button>
                <button
                  ...
                  className={`px-5 py-2 rounded-[var(--radius-sm)] text-xs font-bold transition-all ${currentPhase === "execution" ? "bg-[var(--text-primary)] text-[var(--text-inverted)] shadow-[var(--shadow-sm)]" : "text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]"}`}
                >
                  Exécution
                </button>
              </div>
              ...
              {currentPhase === "execution" ? (
                sc ? (
                  <div className="space-y-3">
                    <div className="mb-2">
                      ...
                      <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5">
                        {sc.activities.length} activité
                        {sc.activities.length !== 1 ? "s" : ""} • Déroulez une
                        activité pour gérer ses dossiers
                      </p>
                    </div>
                    {sc.activities.map((act, aIdx) => {
                      ...
                      return (
                        <div
                          key={aIdx}
                          className="bg-[var(--bg-surface)] rounded-[var(--radius-lg)] border border-[var(--border-default)] shadow-[var(--shadow-sm)] overflow-hidden"
                        >
                          <div
                            onClick={() => toggleRow(aIdx)}
                            className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-[var(--bg-surface-hover)] transition-colors"
                          >
                            <ChevronRight
                              size={14}
                              className={`text-[var(--text-tertiary)] transition-transform duration-200 flex-shrink-0 ${isExpanded ? "rotate-90" : ""}`}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-[10px] text-[var(--text-tertiary)] mt-0.5">
                                {act.budget} • {act.delai}
                              </div>
                            </div>
                            <div className="flex items-center gap-3 flex-shrink-0">
                              <div className="w-20 h-1.5 bg-[var(--bg-inset)] rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${barColor}`}
                                  style={{ width: `${act.pct}%` }}
                                />
                              </div>
                              <span className="text-[11px] font-bold text-[var(--text-primary)] w-8 text-right">
                                {act.pct}%
                              </span>
                              <span
                                className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${statusStyle}`}
                              >
                                {statusText}
                              </span>
                            </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    <div className="w-16 h-16 rounded-full bg-[var(--bg-inset)] flex items-center justify-center mb-4">
                      <FolderOpen
                        size={24}
                    ...
                    <p className="text-[11px] text-[var(--text-tertiary)] mt-1.5 max-w-xs leading-relaxed">
                      Choisissez un sous-composant pour gérer les documents
                      d&apos;exécution.
                    </p>
                  </div>
                )
                ...
                <div className="space-y-4">
                  <div className="mb-2">
                    <h2 className="text-sm font-semibold text-[var(--text-primary)]">
                      {currentPhase === "etude" ? "Étude" : "Passation"} —{" "}
                      {sc ? sc.name : comp?.name}
                    </h2>
                    <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5">
                      Dossiers et exigences documentaires pour cette phase
                    </p>
                  </div>
                  <LivrablesTable
                    docs={currentPhase === "etude" ? etudeDocs : passationDocs}
                    contextTitle={sc ? sc.name : comp!.name}
                    phase={currentPhase}
                  />
                </div>
              )}
